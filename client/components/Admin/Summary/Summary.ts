...
import { AdminSummaryService } from './Service/Summary.service';
...
export class SummaryReadComponent implements OnInit, OnDestroy, SummaryObserver {
    entries: Array<SummaryInterface> = [];
    entry: SummaryInterface = new SummaryInterface();
    total;
    skip = 0;
    limit = 8;
    search;
    count = 0;
    number: Array<SummaryInterface> = [];



    constructor(private repository: AdminSummaryService) {
    ...
    row(value) {
        ...
        this.repository.pagination(this.skip, value)
            .then((entries: Array<SummaryInterface>) => this.entries = entries)
    ...
    page(button) {
        if (button === 'Next' && this.entries.length >= 4) {
            ...
            this.skip += 4;
            this.repository.pagination(this.skip, this.limit).then((entries: Array<SummaryInterface>) => this.entries = entries)
    ...
    ngOnInit() {
        ...
        this.repository.numbering()
            .then((number: Array<SummaryInterface>) => this.number = number)
        // .then((number: Array<SummaryInterface>) => this.total = this.number.length)
