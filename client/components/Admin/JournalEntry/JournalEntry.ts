...
import { AdminJournalEntryService } from './Service/JournalEntry.service';
...
@Component({
    ...
    template: require('./JournalEntry.html'),
    styles: [`
...
export class AdminJournalEntryReadComponent implements OnInit, OnDestroy, JournalEntryObserver {
    entries: Array<JournalEntryInterface> = [];
    ...
    search;
    count = 0;
    number: Array<JournalEntryInterface> = [];



    constructor(private repository: AdminJournalEntryService) {
    ...
    row(value) {
        ...
        this.repository.pagination(this.skip, value)
            .then((entries: Array<JournalEntryInterface>) => this.entries = entries)
    ...
    page(button) {
        if (button === 'Next' && this.entries.length >= 4) {
            ...
            this.skip += 4;
            this.repository.pagination(this.skip, this.limit).then((entries: Array<JournalEntryInterface>) => this.entries = entries)
    ...
    ngOnInit() {
        ...
        this.repository.numbering()
            .then((number: Array<JournalEntryInterface>) => this.number = number)
        // .then((number: Array<JournalEntryInterface>) => this.total = this.number.length)
