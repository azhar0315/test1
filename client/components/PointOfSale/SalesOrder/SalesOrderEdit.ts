...
import { SalesOrderInterface, items } from './Service/SalesOrder.interface';
...
@Component({
    ...
    template: require('./SalesOrderEdit.html'),
    styles: [`
    input.ng-dirty.ng-invalid { border: solid red 2px; }
    input.ng-dirty.ng-valid { border: solid green 2px; }`]

})
export class SalesOrderEditComponent implements OnInit, OnDestroy, SalesOrderObserver {
    ...
    iname;
    ides;
    constructor(private router: Router, private repository: SalesOrderService, private activatedRoute: ActivatedRoute) {
