...
import { SalesOrderInterface, items } from "./SalesOrder.interface";
...
export class SalesOrderService {
    ...
    export(value): Promise<Array<SalesOrderInterface>> {
        if (value === "Json") {
            console.log("3")
    ...
    pagination(skipi): Promise<Array<SalesOrderInterface>> {
        return new Promise((resolve, reject) => {
            let mapFunc: Function = (doc: any) => emit(doc.view);
    ...
    numbering(): Promise<Array<SalesOrderInterface>> {
        return new Promise((resolve, reject) => {
            let mapFunc: Function = (doc: any) => emit(doc.view)
            // ,reduce: '_count';
    ...
    newa(model: SalesOrderInterface): Promise<SalesOrderInterface> {
        return new Promise((resolve, reject) => {
    ...
    hello(model: SalesOrderInterface): Promise<SalesOrderInterface> {

        model.id = Date.now().toString();
        model.view = 'SalesOrder';
    ...
    search(value): Promise<Array<SalesOrderInterface>> {
        return new Promise((resolve, reject) => {
            let options: Object = {
                selector: {
                    $or: [
            ...
            this.pouchDb.find(options)
                .then((result: any) => {
                    let entries: Array<SalesOrderInterface> = result.docs.map((doc: any) => this.mapObjectTolist(doc));
                    resolve(entries);
                })
                .catch(reject);
        });
    ...
    private mapObjectTolist(object: any): SalesOrderInterface {
    ...
    private array(entry: SalesOrderInterface): Object {
        return {
            view: entry.view,
            _id: entry.id,
            _rev: entry.rev,
