...
import { PointOfPurchaseReportsService } from './Service/Reports.service';
import { ReportsObserver } from './Service/Reports.observer';
...
export class PointOfPurchaseReportsReadComponent implements OnInit, OnDestroy, ReportsObserver {
    entries: Array<ReportsInterface> = [];
    entry: ReportsInterface = new ReportsInterface();
    total;
    skip = 0;
    limit = 8;
    search;
    count = 0;
    number: Array<ReportsInterface> = [];



    constructor(private repository: PointOfPurchaseReportsService) {
    ...
    row(value) {
        ...
        this.repository.pagination(this.skip, value)
            .then((entries: Array<ReportsInterface>) => this.entries = entries)
    ...
    page(button) {
        if (button === 'Next' && this.entries.length >= 4) {
            ...
            this.skip += 4;
            this.repository.pagination(this.skip, this.limit).then((entries: Array<ReportsInterface>) => this.entries = entries)
    ...
    ngOnInit() {
        ...
        this.repository.numbering()
            .then((number: Array<ReportsInterface>) => this.number = number)
        // .then((number: Array<ReportsInterface>) => this.total = this.number.length)
