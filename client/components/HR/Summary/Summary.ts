...
import { HRSummaryInterface } from './Service/Summary.interface';
import { HRSummaryService } from './Service/Summary.service';
import { HRSummaryObserver } from './Service/Summary.observer';
...
export class HRSummaryReadComponent implements OnInit, OnDestroy, HRSummaryObserver {
    entries: Array<HRSummaryInterface> = [];
    entry: HRSummaryInterface = new HRSummaryInterface();
    total;
    skip = 0;
    limit = 8;
    search;
    count = 0;
    number: Array<HRSummaryInterface> = [];



    constructor(private repository: HRSummaryService) {
    ...
    row(value) {
        ...
        this.repository.pagination(this.skip, value)
            .then((entries: Array<HRSummaryInterface>) => this.entries = entries)
    ...
    page(button) {
        if (button === 'Next' && this.entries.length >= 4) {
            ...
            this.skip += 4;
            this.repository.pagination(this.skip, this.limit).then((entries: Array<HRSummaryInterface>) => this.entries = entries)
    ...
    ngOnInit() {
        ...
        this.repository.numbering()
            .then((number: Array<HRSummaryInterface>) => this.number = number)
        // .then((number: Array<HRSummaryInterface>) => this.total = this.number.length)
