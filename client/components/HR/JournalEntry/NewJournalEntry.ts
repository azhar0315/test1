...
import { JournalEntryInterface, items } from './Service/JournalEntry.interface';
import { EmployeeInterface } from '../../HR/Employee/Service/Employee.interface';
...
@Component({
    selector: 'NewJournalEntry',
    template: require('./NewJournalEntry.html'),

})
export class HRNewJournalEntryReadComponent implements OnInit, OnDestroy, JournalEntryObserver {
    entry: JournalEntryInterface = new JournalEntryInterface();
    total;
    arr;
    tot;
    Employee: Array<EmployeeInterface> = [];
    ...
    constructor(private repository: HRJournalEntryService) {
    }
    aa(i) {
        console.log("pushed ..!")
        console.log(i)
    ...
    find() {
        console.log(this.entry.Employee)
        if (this.entry.Employee.length > 0) {

            this.repository.searchEmployee(this.entry.Employee)
                .then((Employee: Array<EmployeeInterface>) => this.Employee = Employee)
                .then((Employee: Array<EmployeeInterface>) => console.log(this.Employee))
        }
        if (this.entry.Employee.length == 0) {
            this.repository.fetchEmployee()
                .then((Employee: Array<EmployeeInterface>) => this.Employee = Employee)
                .then((Employee: Array<EmployeeInterface>) => console.log(this.Employee))
